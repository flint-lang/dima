...
class Expression : public dima::Type<Expression> {
  public:
    std::array<double, VALUES_LEN> values; // 512 Bytes of data
...
void apply_complex_operation(dima::Array<Expression> variables) {
...
void apply_simple_operation(dima::Array<Expression> arr) {
...
std::tuple<duration, duration, duration, duration, size_t, size_t, size_t> test_n_allocations(const size_t n) {
    ...
    size_t slot_capacity = 0;
    {
        ...
        dima::Array<Expression> variables = Expression::allocate_array(n);
        for (size_t i = 0; i < variables.size(); i++) {
            dima::Var<Expression> expr = variables[i];
            expr->set_type(std::string("expr_") + std::to_string(i));
        }
